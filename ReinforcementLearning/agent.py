...
class Agent:
    ...
    def remember(self, state, action, reward, next_state, done):
        self.memory.append((state, action, reward, next_state, done))  # popleft if memory exceeds
    ...
    def get_action(self, state):
        ...
        if random.randint(0, 200) < self.epsilon:
            ...
            state0 = torch.tensor(state, dtype=torch.float).to(self.device)  # Move tensor to the device
